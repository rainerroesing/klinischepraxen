...
<div class="uk-grid uk-grid-medium uk-grid-match<?= $tabletWidth ?><?= $columnWidth ?>" uk-grid>
<?php foreach ($articles as $article) : ?>
<?php if($data->articlesImage() == 'true') : ?>
<div>
<article class="uk-card-small uk-transition-toggle <?php e($data->articlesColor() == "muted", ' uk-card-default') ?><?php e($data->articlesColor() == "primary", ' uk-card-primary') ?><?php e($data->articlesColor() == "secondary", ' uk-card-secondary') ?>" tabindex="0">
...
<a href="<?= $article->url() ?>">
  <picture>
    ...
    <img class="uk-transition-scale-up uk-transition-opaque" src="<?= $img->crop(500, 400)->url() ?>" alt="<?= $article->title()->html() ?>" width="500" height="400" loading="lazy" style="border-radius: 10px;">
  </picture>
</a>
</div>
<?php endif ?>
<div class="uk-card-body">
<p class="uk-article-meta uk-margin-small"><time><?= $article->date()->html()->toDate($article->parent()->dateFormat()->html()) ?></time></p>
<h3<?php e($data->articleTitleSize()->isNotEmpty(), ' class="' . $data->articleTitleSize() . '"') ?> style="font-weight: bold; margin-bottom: 10px;"><a href="<?= $article->url() ?>"><?= $article->title()->html() ?></a></h3>
